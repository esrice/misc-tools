...
def main():
    ...
    for ref_name, ref_length in zip(args.bamfile.references,
                                    args.bamfile.lengths):
        total_aligned_length = 0
        for segment in args.bamfile.fetch(ref_name):
            if not (segment.is_secondary
                    or segment.is_supplementary
                    or segment.is_unmapped):
                total_aligned_length += segment.query_alignment_length
        print('\t'.join([
            ref_name,
            str(total_aligned_length/ref_length),
            str(ref_length),
        ]))
