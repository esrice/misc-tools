...
def parse_args():
    parser = argparse.ArgumentParser(description='Given a bed file containing '
            'the mapping coordinates of Hi-C reads, make a heatmap.')
    ...
    parser.add_argument('contigs_list', type=argparse.FileType('r'),
...
def heat_coords_from_bed(bed_file, contigs, contigs_index):
    ...
    heat_coords_x = []
    heat_coords_y = []
    last_contig, last_start, last_end, last_read_name = '', -1, -1, ''
    for line in bed_file:
        contig, start, end, read_name, mapq, orient = line.strip().split()
        ...
        if read_name.endswith('/1'):
            ...
            last_end, last_read_name = end, read_name
            ...
            if last_coord != None and this_coord != None:
