...
def parse_args():
    ...
    parser.add_argument('-m', '--min-coverage', type=int, default=10,
            help='minimum read coverage required to consider a position [10]')
    parser.add_argument('-M', '--max-coverage', type=int, default=100,
    ...
    parser.add_argument('-h', '--min-het-pct', type=float, default=0.25,
            help='minimum portion of reads supporting alternate allele for '
            'a position to be considered heterozygous [0.25]')
    parser.add_argument('-H', '--max-het-pct', type=float, default=0.75,
...
def parse_pileup(instream, min_coverage, max_coverage):
    ...
    """
    for line in map(lambda b: b.decode("ascii"), instream):
...
def main():
    for pile in parse_pileup(sys.stdout, args.min_coverage, args.max_coverage):
