...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Make a circos plot from " "mashmap alignments."
    )
    parser.add_argument(
        "mashmap", type=argparse.FileType("r"), help="output of mashmap"
    )
    parser.add_argument(
        "-p", "--output-prefix", default="out", help="prefix for output files [out]"
    )
...
def sort_sequence_names(sequence_names):
    ...
    chromosome_lambda = (
        lambda s: str.starts_with(s, "chr") or s.isdigit() or s in "WXYZM"
    )
...
def mash_to_circos(
    ...
    * query_color, reference_color: colors to use for
    ...
    for reference_name in sorted(
        reference_seq_sizes.keys(), key=chr_sort_key, reverse=True
    ):
