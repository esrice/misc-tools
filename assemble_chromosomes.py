...
def parse_args():
    ...
    parser.add_argument(
        "scaffolds_fasta",
        type=pyfaidx.Fasta,
        help="a fasta " "file containing scaffolds to be assembled into chromosomes",
    )
    parser.add_argument(
        "layout_file",
...
def main():
    ...
    for line_number, line in enumerate(args.layout_file):
        try:  # make sure there are the right number of columns
            ...
            print(
                "FATAL: Misformatted layout file on line {}".format(line_number + 1),
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        for scaffold_number, scaffold in enumerate(scaffolds):
            ...
            try:
                ...
                eprint('FATAL: no sequence with ID "{}" in input'.format(scaffold.name))
                sys.exit(1)
        ...
        print(
            SeqRecord(
                Seq(chromosome_sequence), id=chromosome_name, description=""
            ).format("fasta"),
            end="",
        )
    ...
    for scaffold in args.scaffolds_fasta:
        if scaffold.name not in assigned_scaffolds:
            print(
                SeqRecord(
                    Seq(scaffold[:].seq), id=scaffold.name, description=""
                ).format("fasta"),
                end="",
            )
